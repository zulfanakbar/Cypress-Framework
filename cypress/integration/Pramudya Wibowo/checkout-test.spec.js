describe('Checkout Test SauceDemo', () => {
    ...
    it('Normal Checkout with 2 item', () => {
        cy.visit('https://www.saucedemo.com/')
        cy.get('#user-name').type('standard_user').should('have.value', 'standard_user')
        cy.get('#password').type('secret_sauce').should('have.value', 'secret_sauce')
        cy.get('#login-button').click()
        ...
        cy.get('#shopping_cart_container > a').click()
        cy.url().should('include', '/cart.html')
        cy.get("#cart_contents_container > div > div.cart_list > div:nth-child(3) > div.cart_item_label > div.item_pricebar > div").invoke('text').then((price1) => {
            cy.get('#cart_contents_container > div > div.cart_list > div:nth-child(4) > div.cart_item_label > div.item_pricebar > div').invoke('text').then((price2) => {
                const actualprice = Number(price1.replace(/[^0-9.-]+/g,"")) + Number(price2.replace(/[^0-9.-]+/g,""))
                cy.get('#checkout').click()
                cy.url().should('include', '/checkout-step-one.html')
                cy.get('#first-name').type('First Name').should('have.value', 'First Name')
                cy.get('#last-name').type('Last Name').should('have.value', 'Last Name')
